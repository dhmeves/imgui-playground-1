...
static nfdresult_t SetDefaultPath( IFileDialog *dialog, const char *defaultPath )
{
    if ( !defaultPath || strlen(defaultPath) == 0 )
        return NFD_OKAY;
    printf("defaultPath FOUND: %s\n", defaultPath);   //  TODO - DELETE ME
    printf("(PCWSTR)defaultPath FOUND : % s\n", (PCWSTR)defaultPath);   //  TODO - DELETE ME
    wchar_t *defaultPathW = {0};
    CopyNFDCharToWChar( defaultPath, &defaultPathW );
    printf("defaultPathW FOUND: %ws\n", defaultPathW);   //  TODO - DELETE ME
    IShellItem *folder;
    ...
    {
        printf("error, default not found: Error#%u\n", result);//  TODO - DELETE ME
